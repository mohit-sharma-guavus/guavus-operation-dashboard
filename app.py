...
#Config Parameters
app.config["CLIENT_CSV"] = "/Users/navneet.singh/Desktop/Navneet/PX/guavus-operation-dashboard-mohit-dev/scripts"
    ...
    if request.method == 'POST':
        if not request.form['email'] or not request.form['description']:
         return('Please enter cluster name the fields', 'error')
            ...
            conn.commit()
            print('Cluster was successfully added!!!')
            conn = mysql.connect()
            cursor = conn.cursor()
...
def in_progress():
    print("Showing In Progress page")
